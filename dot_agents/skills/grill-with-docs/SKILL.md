...
Keep it tight — one section per decision, no filler. The "Domain Updates" section references the `CONTEXT.md` changes and ADRs that were written inline during the session, so the next agent (spec or planner) knows what documentation was touched.

This document is the handoff to whoever acts on the decisions.
